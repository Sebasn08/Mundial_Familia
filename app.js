...
function renderAdminBracket(container){
  ...
  // qualifiedThirdGroups se sincroniza desde adminThirdPicks antes de construir el mapa
  if(!STATE.knockout.adminThirdPicks) STATE.knockout.adminThirdPicks = {};
  STATE.knockout.qualifiedThirdGroups = Object.values(STATE.knockout.adminThirdPicks).filter(Boolean);
  ...
  function buildThirdsPickerHtml(){
    if(!STATE.knockout.adminThirdPicks) STATE.knockout.adminThirdPicks = {};
    const picks = STATE.knockout.adminThirdPicks;

    const SLOT_RIVAL = {"3#1":"1E","3#2":"1I","3#3":"1D","3#4":"1G","3#5":"1A","3#6":"1L","3#7":"1B","3#8":"1K"};
    const SLOTS = Object.keys(SLOT_RIVAL);

    const usedGroups = new Set(Object.values(picks).filter(Boolean));
    const assignedCount = usedGroups.size;
    const unassignedGroups = realThirds.filter(r => !usedGroups.has(r.group));

    // Sincronizar qualifiedThirdGroups con los picks actuales (para buildThirdSlotMap)
    STATE.knockout.qualifiedThirdGroups = Object.values(picks).filter(Boolean);

    const thirdsMapAll = {};
    realThirds.forEach(r => thirdsMapAll[r.group] = r);

    let html = '<div class="card" style="margin-bottom:14px;">';
    html += '<h3>Terceros clasificados — asignación de llaves</h3>';
    html += '<p style="font-size:13px;color:var(--muted);margin:4px 0 12px;">Asigná cada uno de los 8 mejores terceros a su llave. Cada grupo sólo puede usarse una vez. El cruce exacto (3#1..3#8) sigue el Anexo C del reglamento oficial.</p>';

    // Indicador de progreso
    html += '<div style="margin-bottom:14px;padding:8px 12px;border-radius:8px;background:'+(assignedCount===8?'#e8f5e9':'#fdf8ee')+';border:1px solid '+(assignedCount===8?'#a5d6a7':'var(--line)')+';">';
    html += '<span style="font-size:13px;font-weight:600;color:'+(assignedCount===8?'#2b7a47':'var(--gold-dark)')+';">'+assignedCount+'/8 llaves asignadas'+(assignedCount===8?' ✓':'')+'</span>';
    if(assignedCount < 8) html += '<span style="font-size:12px;color:var(--muted);margin-left:8px;">Faltan '+(8-assignedCount)+'</span>';
    html += '</div>';

    // Tabla de referencia de todos los terceros reales
    html += '<details style="margin-bottom:12px;"><summary style="font-size:13px;color:var(--gold-dark);cursor:pointer;font-weight:600;">Ver todos los terceros reales ▾</summary>';
    html += '<div style="margin-top:8px;">';
    realThirds.forEach((row, idx)=>{
      const isAssigned = usedGroups.has(row.group);
      html += '<div class="third-row" style="opacity:'+(isAssigned?'0.5':'1')+'">';
      html += '<span><span class="third-rank">'+(idx+1)+'°</span>'+escapeHtml(row.team)+' <span style="color:var(--muted);font-size:11px;">(Grupo '+row.group+')</span></span>';
      html += '<span style="color:var(--muted);">'+row.pts+' pts &middot; DG '+(row.dg>0?'+':'')+row.dg+'</span>';
    ...
    // Desplegable por cada slot 3#1..3#8
    SLOTS.forEach(slot=>{
      const currentPick = picks[slot] || "";
      const rival = SLOT_RIVAL[slot];
      ...
      html += '<span style="min-width:32px;font-family:Oswald,sans-serif;font-size:12px;color:var(--gold-dark);font-weight:600;">'+slot+'</span>';
      html += '<span style="min-width:38px;font-size:12px;color:var(--muted);">vs '+rival+'</span>';
      html += '<select class="text-input admin-third-pick-select" data-slot="'+slot+'" style="flex:1;font-size:13px;padding:4px 8px;">';
      html += '<option value="">-- Elegir 3° --</option>';

      realThirds.forEach(r=>{
        const isUsedElsewhere = usedGroups.has(r.group) && currentPick !== r.group;
        const disabled = isUsedElsewhere ? 'disabled' : '';
        const selected = currentPick === r.group ? 'selected' : '';
        const style = isUsedElsewhere ? 'color:var(--muted);' : '';
      ...
      html += '</select>';
      if(currentPick && thirdsMapAll[currentPick]){
        html += '<span style="font-size:12px;font-weight:600;color:#2b7a47;min-width:60px;">✓ '+escapeHtml(thirdsMapAll[currentPick].team)+'</span>';
      } else {
        html += '<span style="min-width:60px;"></span>';
      }
      html += '</div>';
    ...
    // Mostrar los que quedan sin asignar
    if(assignedCount > 0){
      html += '<div style="margin-top:12px;padding-top:10px;border-top:1px solid var(--line);">';
      html += '<span style="font-size:12px;color:var(--muted);font-weight:600;">Sin asignar ('+unassignedGroups.length+'): </span>';
      unassignedGroups.forEach(r=>{
    ...
    html += '<button class="btn secondary" id="suggestThirdsBtn" style="margin-top:12px;">Sugerir automáticamente (top 8 por ranking)</button>';
    html += '</div>';
    return html;
  }
  ...
  KNOCKOUT_ROUNDS.forEach((round, ridx)=>{
    ...
    STATE.knockout[round.key].forEach((m, midx)=>{
      ...
      html += '<span style="font-size:11px;color:var(--muted);margin-left:8px;">resultado 90&prime;</span>';
      html += '</div>';
      // Selector de ganador por penales: aparece cuando el resultado es empate
  ...
  // ---- Picker admin de terceros (desplegables) ----
  container.querySelectorAll('.admin-third-pick-select').forEach(sel=>{
    sel.addEventListener('change', ()=>{
      const slot = sel.dataset.slot;
      const val = sel.value;
      if(!STATE.knockout.adminThirdPicks) STATE.knockout.adminThirdPicks = {};
      if(val) STATE.knockout.adminThirdPicks[slot] = val;
      else delete STATE.knockout.adminThirdPicks[slot];
      // Sincronizar qualifiedThirdGroups y recalcular
      STATE.knockout.qualifiedThirdGroups = Object.values(STATE.knockout.adminThirdPicks).filter(Boolean);
  ...
  if(suggestBtn){
    suggestBtn.addEventListener('click', ()=>{
      const top8 = computeRealThirds().slice(0, 8);
      STATE.knockout.adminThirdPicks = {};
      const slots = ["3#1","3#2","3#3","3#4","3#5","3#6","3#7","3#8"];
      top8.forEach((t, i) => { STATE.knockout.adminThirdPicks[slots[i]] = t.group; });
      STATE.knockout.qualifiedThirdGroups = top8.map(t => t.group);
      renderAdminBracket(container);
      showToast("Sugerencia aplicada según ranking actual");
    });
  }

  // Mostrar/ocultar el selector de penales en tiempo real al cambiar el marcador
  container.querySelectorAll('.bracket-match').forEach(el=>{
    ...
    function updatePenRow(){
      ...
      if(!isTie){
        // Limpiar selección si ya no hay empate
        el.querySelectorAll('.admin-pen-radio').forEach(r => r.checked = false);
      }
    }
...
function readBracketFromDOM(container){
  container.querySelectorAll('.bracket-match').forEach(el=>{
    ...
    // Para r32, los campos home/away son readonly (autocompletados desde grupos),
    // NO los leemos del DOM para evitar invertir equipos al guardar.
    // autoFillR32FromGroups() es la fuente de verdad para esos slots.
    if(round !== 'r32'){
