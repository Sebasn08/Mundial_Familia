...
function buildBracketTreeHtml(bracket, opts){
  ...
  function slotBadge(slot){
    ...
    if(slot.startsWith('3#')){
      // Tercero: comparar thirdPicks del participante vs adminThirdPicks
      ...
      if(userGroup === adminGroup)
        return '<span style="font-size:11px;color:#2b7a47;margin-left:3px;" title="¡Acertaste el tercero de esta llave!">✅</span>';
      return '<span style="font-size:11px;color:#c0392b;margin-left:3px;" title="No acertaste el tercero de esta llave">❌</span>';
    }
...
function renderPredKnockoutForm(){
  ...
  SLOTS.forEach(slot=>{
    ...
    if(currentPick && thirdsMapAll[currentPick]){
      // Comparar con lo que el admin estableció como oficial
      const adminPicks = (STATE.knockout && STATE.knockout.adminThirdPicks) || {};
      const adminPick = adminPicks[slot];
      let badge = '';
      if(adminPick){
        // El admin ya definió este slot: podemos calificar
        if(currentPick === adminPick){
          badge = '<span style="font-size:14px;margin-left:4px;" title="¡Acertaste el tercero de esta llave!">✅</span>';
        } else {
          badge = '<span style="font-size:14px;margin-left:4px;" title="El tercero oficial es '+escapeHtml(thirdsMapAll[adminPick]?thirdsMapAll[adminPick].team:adminPick)+' (Grupo '+adminPick+')">❌</span>';
        }
      }
...
function calculateTotalScore(pred){
  ...
  ['3#1','3#2','3#3','3#4','3#5','3#6','3#7','3#8'].forEach(slot=>{
    if(adminThirdPicks[slot] && userThirdPicks[slot] && adminThirdPicks[slot] === userThirdPicks[slot]){
