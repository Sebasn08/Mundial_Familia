...
function resolveUserR32(pred){
  ...
  function resolveSlot(slot){
    if(slot.startsWith("3#")){
      const n = parseInt(slot.slice(2)) - 1;
      const row = best8[n];
      if(!row) return "";
      // si el grupo de ese tercero no está completo, igual mostramos el equipo proyectado
      return row.team || "";
    }
...
function renderPredKnockoutForm(){
  ...
  // Tabla de los 12 terceros ordenados, marcando los 8 que clasifican en esta proyección
  html += '<div class="card bracket-thirds-card" style="margin-bottom:14px;">';
  html += '<h3 style="margin:0 0 8px;">Mejores terceros (según tu proyección)</h3>';
  ...
  html += '<p class="bracket-legend" style="margin:8px 0 0;">Resaltados: los 8 que clasificarían a dieciseisavos según tu proyección actual.</p>';
  html += '</div>';
...
function renderAdminBracket(container){
  ...
  autoFillR32FromGroups();

  // Armar lista de todos los terceros reales ordenados por ranking (para el select)
  ...
  function thirdsSelect(value, side){
    let s = '<select class="text-input" data-side="'+side+'">';
    s += '<option value="">-- Elegir mejor tercero --</option>';
    if(realThirds.length === 0){
      s += '<option disabled>Grupos incompletos aún</option>';
    } else {
      realThirds.forEach((t, i)=>{
        s += '<option value="'+escapeHtml(t.team)+'" '+(value===t.team?'selected':'')+'>'+escapeHtml((i+1)+'° '+t.team+' (Gr.'+t.group+', '+t.pts+' pts)')+'</option>';
      });
    }
    s += '</select>';
    return s;
  ...
  function autoDisplay(team, slot, side){
    // Muestra el equipo como texto fijo (autocompletado) con input hidden para que readBracketFromDOM lo lea
    return '<div class="text-input" style="background:#f0ede3;color:var(--pitch);font-weight:500;display:flex;align-items:center;gap:6px;">'
      + '<span>'+escapeHtml(team||slot)+'</span>'
      + (team ? '<span style="font-size:10px;color:var(--gold-dark);white-space:nowrap;">✓ auto</span>' : '')
      + '<input type="hidden" data-side="'+side+'" value="'+escapeHtml(team||'')+'"></div>';
  }

  let html = '<div class="card">';
  html += '<h3>Cuadro de eliminatorias</h3>';
  html += '<p style="font-size:13px;color:var(--muted);margin:4px 0 16px;">Los 1° y 2° de cada grupo se completan automáticamente al tener todos sus resultados. Los terceros clasificados se eligen de la lista según su ranking real. Las rondas siguientes se propagan con "Autocompletar".</p>';
