...
function renderPredGroupContent(letter, pred){
  ...
  if(filledCount===0){
    ...
    if(predQ.first || predQ.second){
      const realStandings = computeGroupStandings(letter);
      const realComplete = isGroupComplete(letter);
      const actualFirst  = realComplete ? realStandings[0].team : null;
      const actualSecond = realComplete ? realStandings[1].team : null;
      // Calcular puntos para cada badge
      const ptsFirst  = !actualFirst  ? null
        : predQ.first  === actualFirst  ? STATE.points.campeonGrupo
        : [actualFirst, actualSecond].includes(predQ.first)  ? STATE.points.dosClasificados : 0;
      const ptsSecond = !actualSecond ? null
        : predQ.second === actualSecond ? STATE.points.segundoGrupo
        : [actualFirst, actualSecond].includes(predQ.second) ? STATE.points.dosClasificados : 0;
      html += '<div style="margin-top:12px;padding-top:10px;border-top:1px solid var(--line);">';
      html += '<span style="font-size:11px;color:var(--muted);text-transform:uppercase;letter-spacing:.06em;">Clasificados predichos: </span>';
      if(predQ.first)  html += buildQualifierBadge(predQ.first,  actualFirst,  '1°', ptsFirst);
      if(predQ.second) html += buildQualifierBadge(predQ.second, actualSecond, '2°', ptsSecond);
      html += '</div>';
    }
