...
function buildBracketTreeHtml(bracket, opts){
  ...
  // ---- Layout con posicionamiento absoluto y conectores SVG ----
  // CELL_H y SEP deben coincidir con --bch y --bsep en CSS.
  const CELL_H = 32;
  const SEP    = 2;
  const PAIR_H = CELL_H * 2 + SEP; // 66px

  const roundCounts = {r32:16, r16:8, qf:4, sf:2, final:1};
  const totalH = roundCounts.r32 * PAIR_H; // 1056px

  function slotH(rkey){ return totalH / (roundCounts[rkey]||1); }
  function pairTop(rkey, i){ const s=slotH(rkey); return i*s+(s-PAIR_H)/2; }
  function pairCY(rkey, i){ return pairTop(rkey,i)+PAIR_H/2; }

  const ROUND_COLOR_CLASS = {r32:'bcol-r32',r16:'bcol-r16',qf:'bcol-qf',sf:'bcol-sf',final:'bcol-final',third:'bcol-final'};
  const COL_W  = 160; // px, debe coincidir con --bcw
  const GAP_W  = 28;  // px entre columnas (espacio para conector)

  let html = '<div class="bracket-scroll"><div class="bracket-grid" style="display:flex;flex-direction:row;align-items:flex-start;gap:0;padding:0 0 16px 0;">';
  ...
  mainRounds.forEach((round, roundIdx)=>{
    ...
    html += '<div class="bracket-col '+colorClass+'" style="width:'+COL_W+'px;flex-shrink:0;display:flex;flex-direction:column;">';
    html += '<div class="bracket-col-title">'+round.label+'</div>';
    html += '<div style="position:relative;height:'+totalH+'px;flex-shrink:0;">';

    matches.forEach((m,idx)=>{
      const top = pairTop(round.key,idx);
      html += '<div style="position:absolute;top:'+top+'px;left:4px;right:4px;">';
      ...
      if(opts.pointsFn&&m.home&&m.away&&m.result){
        ...
        if(pts!==null) html+='<div style="position:absolute;top:'+(top+PAIR_H+1)+'px;left:4px;font-size:10px;color:var(--gold-dark);font-weight:600;">+'+pts+' pts</div>';
      }
    });

    // SVG de conectores: dibuja las líneas que unen pares de esta ronda con los de la siguiente
    if(!isLastMain){
      const nextRound = mainRounds[roundIdx+1];
      const svgW = GAP_W + 4;
      html += '<svg style="position:absolute;top:0;right:-'+svgW+'px;width:'+svgW+'px;height:'+totalH+'px;overflow:visible;pointer-events:none;" xmlns="http://www.w3.org/2000/svg">';
      matches.forEach((m,idx)=>{
        const thisCY = pairCY(round.key,idx);
        const nextIdx = Math.floor(idx/2);
        const nextCY  = pairCY(nextRound.key,nextIdx);
        const x1 = 0, x2 = svgW;
        const midX = svgW/2;
        // Línea horizontal desde el par actual al punto medio, luego vertical al par siguiente, luego horizontal
        // Forma de "bracket": ─┐ o ─┘ dependiendo si es par (top) o impar (bottom)
        // Línea horizontal de salida del par (siempre)
        html += '<line x1="0" y1="'+thisCY+'" x2="'+midX+'" y2="'+thisCY+'" stroke="#bdb5a0" stroke-width="1.5"/>';
        if(idx%2===0){
          // Par par (superior del grupo de 2): dibuja la vertical que une ambos hermanos
          // y la horizontal de llegada al par de la ronda siguiente.
          // nextCY es el centro del par siguiente; la vertical va de thisCY a nextCY pero
          // la dibujamos completa solo desde el par par (el impar la completaría igual).
          const nextOddCY = pairCY(nextRound.key, nextIdx); // mismo nextIdx para ambos hermanos
          // Línea vertical desde este par hasta la posición del par impar (idx+1)
          const siblingCY = pairCY(round.key, idx+1);
          html += '<line x1="'+midX+'" y1="'+thisCY+'" x2="'+midX+'" y2="'+siblingCY+'" stroke="#bdb5a0" stroke-width="1.5"/>';
          // Línea horizontal hacia la ronda siguiente
          html += '<line x1="'+midX+'" y1="'+nextOddCY+'" x2="'+svgW+'" y2="'+nextOddCY+'" stroke="#bdb5a0" stroke-width="1.5"/>';
        }
      });
      html += '</svg>';
    }
    ...
    // Gap entre columnas
    if(!isLastMain){
      html += '<div style="width:'+GAP_W+'px;flex-shrink:0;"></div>';
    }
  });
  ...
  html += '<div style="width:'+GAP_W+'px;flex-shrink:0;"></div>';
  html += '<div class="bracket-col bracket-final-col bcol-final" style="width:'+COL_W+'px;flex-shrink:0;display:flex;flex-direction:column;">';
  html += '<div class="bracket-col-title">Final</div>';
  html += '<div style="position:relative;height:'+totalH+'px;flex-shrink:0;">';
  finalMatches.forEach((m,idx)=>{
    const top=pairTop('final',idx);
    html += '<div style="position:absolute;top:'+top+'px;left:4px;right:4px;">';
  ...
  html += '<div class="bracket-champion" style="height:'+totalH+'px;">';
  html += '<div class="ctrophy">&#127942;</div>';
  ...
  if(third&&(third.home||third.away)){
    ...
    html += '<div class="bracket-col-title bcol-final" style="width:'+COL_W+'px;border-radius:6px 6px 0 0;padding:5px 6px 6px;text-align:center;">Tercer puesto</div>';
    ...
    if(opts.pointsFn&&third.home&&third.away&&third.result){
      ...
      if(pts!==null) html+='<div style="font-size:10px;color:var(--gold-dark);font-weight:600;margin:3px 0 0 4px;">+'+pts+' pts</div>';
    }
    html += '</div>';
  }
