...
async function initApp(){
  ...
  // Mantener STATE.matches / STATE.knockout sincronizados en vivo: si el admin
  // carga un resultado mientras un participante tiene la app abierta, su sesión
  // se entera al instante (sin recargar) y el marcador se bloquea automáticamente.
  setupLiveSync();
}

// ===================== SINCRONIZACIÓN EN VIVO (admin -> participantes) =====================
// Escucha cambios en admin:matches / admin:knockout / admin:points en Firestore.
// Cuando el admin guarda un resultado (incluso parcial, en vivo), todas las sesiones
// abiertas reciben el cambio de inmediato: esto es lo que evita que un participante
// pueda seguir editando su predicción después de que el partido arrancó.
async function setupLiveSync(){
  ...
  function watch(key, onChange){
    ...
    fs.onSnapshot(ref, (snap)=>{
      ...
      let parsed;
      try{ parsed = JSON.parse(data.value); } catch(e){ return; }
      onChange(parsed);
    }, (err)=>{ console.error("onSnapshot("+key+") error", err); });
  }

  watch("admin:matches", (matches)=>{
    STATE.matches = matches;
    refreshActiveView();
  });

  watch("admin:knockout", (knockout)=>{
    STATE.knockout = knockout;
    refreshActiveView();
  });

  watch("admin:points", (points)=>{
    STATE.points = Object.assign({}, DEFAULT_POINTS, points);
    refreshActiveView();
  });
}

// Vuelve a renderizar la vista que el participante tiene abierta en este momento,
// para que cualquier marcador recién bloqueado por el admin se refleje sin recargar.
function refreshActiveView(){
  const activeView = document.querySelector(".view.active");
  if(!activeView) return;

  if(activeView.id === "view-predicciones"){
    // Si el participante está en medio de escribir una predicción que TODAVÍA no
    // está bloqueada, conservamos lo que tenga tipeado antes de re-renderizar.
    if(STATE.currentParticipant && STATE.predictions) collectPredFormInputs();
    if(predSection === "grupos") renderPredFormGroupContent(predFormGroupLetter);
    else renderPredKnockoutForm();
  } else if(activeView.id === "view-eliminatorias"){
    renderKnockout();
  } else if(activeView.id === "view-tabla"){
    renderLeaderboard();
  }
  // La vista de admin no se auto-refresca: si el propio admin está editando,
  // no queremos pisarle los inputs que tiene abiertos.
}
...
function resolveUserR32(pred){
  ...
  const all12 = thirdsInfo.all;

  // Mapa grupo -> equipo para todos los 12 terceros
  const thirdsMapAll = {};
  all12.forEach(r => thirdsMapAll[r.group] = r);

  const manualPicks = pred.thirdPicks || {};
  // Fallback: usar tabla oficial FIFA (igual que el admin) en vez del índice por ranking
  const qualifiedGroups = thirdsInfo.best8.map(r => r.group);
  const officialThirdSlotMap = buildThirdSlotMap(qualifiedGroups, thirdsInfo.best8);

  function resolveSlot(slot){
    if(slot.startsWith("3#")){
      const pickedGroup = manualPicks[slot];
      if(pickedGroup && thirdsMapAll[pickedGroup]){
        return thirdsMapAll[pickedGroup].team;
      }
      // Fallback al mapa oficial FIFA en vez del índice por ranking
      const autoRow = officialThirdSlotMap[slot];
      return (autoRow && autoRow.team) ? autoRow.team : "";
    }
    const pos = slot[0];
    const letter = slot[1];
...
function computeUserBracket(pred){
  ...
  for(let r=0; r<KNOCKOUT_ROUNDS.length; r++){
    ...
    const winners = prevMatches.map((m, i)=>{
      ...
      if(ph===pa){
        // Empate: usar penaltyWinner si fue elegido
        return (p.penaltyWinner && (p.penaltyWinner===m.home || p.penaltyWinner===m.away))
          ? p.penaltyWinner : "";
      }
  ...
  const sfLosers = sf.map((m,i)=>{
    ...
    if(isNaN(ph) || isNaN(pa)) return "";
    if(ph===pa){
      if(!p.penaltyWinner) return "";
      return p.penaltyWinner === m.home ? m.away : m.home;
    }
...
function buildBracketTreeHtml(bracket, opts){
  ...
  // Badge ✅/❌ para celdas de terceros en r32
  // Devuelve badge ✅/❌ comparando la prediccion del participante vs el resultado real.
  // Soporta slots "1X"/"2X" (primero/segundo de grupo) y "3#N" (tercero en llave).
  function slotBadge(slot){
    if(!slot) return '';

    if(slot.startsWith('3#')){
      // Tercero: comparar el EQUIPO real del grupo elegido por el participante
      // vs el EQUIPO real del grupo elegido por el admin (no basta coincidir en letra de grupo)
      const adminPicks = (STATE.knockout && STATE.knockout.adminThirdPicks) || {};
      const userGroup  = (opts.thirdPicks || {})[slot];
      const adminGroup = adminPicks[slot];
      if(!adminGroup || !userGroup) return '';
      // Si los grupos aún no están completos, no se puede comparar equipos
      if(!isGroupComplete(adminGroup) || !isGroupComplete(userGroup)) return '';
      const adminStandings = computeGroupStandings(adminGroup);
      const userStandings  = computeGroupStandings(userGroup);
      const adminTeam = adminStandings[2] && adminStandings[2].team;
    ...
    if(slot.length === 2 && (slot[0] === '1' || slot[0] === '2')){
      // Primero o segundo de grupo: comparar proyeccion del participante vs standings reales
      const pos    = slot[0]; // "1" o "2"
      const letter = slot[1]; // letra del grupo
      if(!isGroupComplete(letter)) return ''; // grupo no terminado aun
      const realStandings = computeGroupStandings(letter);
      if(!realStandings || realStandings.length < 2) return '';
      const realTeam = pos === '1' ? realStandings[0].team : realStandings[1].team;
      // El equipo que el participante predijo para este slot viene en opts.r32slots
  ...
  function cellHtml(team, round, idx, side, m){
    const slot = (round === 'r32' && m) ? (side==='home' ? m.homeSlot : m.awaySlot) : null;
    const badge = slotBadge(slot);
    const label = team ? '' : '<span style="color:var(--muted);">Por definir</span>';
    const isTie = m && m.result && m.result.home === m.result.away;
    const penWinner = m && m.result && m.result.penaltyWinner;
    let winnerClass = '';
    if(m && m.result){
      let isWinner, isLoser;
      if(isTie && penWinner){
        isWinner = (penWinner === (side==='home' ? m.home : m.away));
        isLoser  = !isWinner && !!(side==='home' ? m.home : m.away);
      } else {
        isWinner = (side==='home' && m.result.home > m.result.away) || (side==='away' && m.result.away > m.result.home);
        isLoser  = (side==='home' && m.result.home < m.result.away) || (side==='away' && m.result.away < m.result.home);
      }
      if(isWinner) winnerClass = 'winner';
      else if(isLoser) winnerClass = 'loser';
    }
    let scoreHtml = '';
    const adminM = opts.adminKnockout && opts.adminKnockout[round] && opts.adminKnockout[round][idx];
    const adminHasResult = adminM && adminM.result != null &&
      adminM.result.home !== '' && adminM.result.home !== undefined &&
      adminM.result.away !== '' && adminM.result.away !== undefined;
    if(editable && team && !adminHasResult){
      ...
      scoreHtml = '<span class="bscore">'+(side==='home'?m.result.home:m.result.away)+(adminHasResult?'<span style="font-size:9px;margin-left:2px;" title="Resultado oficial">🔒</span>':'')+'</span>';
    }
    // Nombre del equipo: si hay empate y es editable, clic para elegir ganador por penales
    let teamLabel;
    if(team && editable && isTie && !adminHasResult){
      const isPenWin = penWinner === team;
      const penStyle = isPenWin
        ? 'cursor:pointer;border-bottom:2px solid var(--gold-dark);font-weight:700;'
        : 'cursor:pointer;opacity:0.75;';
      const penTitle = isPenWin ? 'Ganador por penales (clic para desmarcar)' : 'Clic para marcar como ganador por penales';
      const penBadge = isPenWin
        ? ' <span style="font-size:10px;vertical-align:middle;">&#x1F945;</span>'
        : ' <span style="font-size:10px;color:var(--muted);vertical-align:middle;">pen?</span>';
      teamLabel = '<span class="bteam bteam-pen" title="'+penTitle
        +'" data-round="'+round+'" data-idx="'+idx+'" data-team="'+escapeHtml(team)+'" style="'+penStyle+'">'
        +escapeHtml(team)+penBadge+badge+'</span>';
    } else if(team){
      teamLabel = '<span class="bteam" title="'+escapeHtml(team)+'">'+escapeHtml(team)+badge+'</span>';
    } else {
      teamLabel = '<span class="bteam">'+label+'</span>';
    }
    return '<div class="bracket-cell '+winnerClass+'" data-round="'+round+'" data-idx="'+idx+'" data-side="'+side+'">'+teamLabel+scoreHtml+'</div>';
  }

  let html = '<div class="bracket-scroll"><div class="bracket-grid">';

  // Mapa de clase de color por ronda
  const ROUND_COLOR_CLASS = {r32:'bcol-r32', r16:'bcol-r16', qf:'bcol-qf', sf:'bcol-sf', final:'bcol-final', third:'bcol-final'};

  // Columnas r32 -> sf
  const mainRounds = KNOCKOUT_ROUNDS.filter(r=>r.key!=="third" && r.key!=="final");
  mainRounds.forEach(round=>{
    const matches = bracket[round.key] || [];
    const colorClass = ROUND_COLOR_CLASS[round.key] || '';
    html += '<div class="bracket-col '+colorClass+'"><div class="bracket-col-title">'+round.label+'</div><div class="bracket-pairs">';
  ...
  html += '<div class="bracket-col bracket-final-col bcol-final"><div class="bracket-col-title">Final</div><div class="bracket-pairs">';
...
// real todavía no se jugó (no hay nada que mostrar todavía).
// - Si la llave coincide: puntúa clasificado + marcador normalmente.
// - Si la llave NO coincide pero el participante eligió al ganador real como
//   ganador en su cruce: otorga solo los puntos de clasificado con aclaración.
// - Si la llave NO coincide y tampoco acertó el ganador: 0 pts.
function knockoutPointsFn(round, userM, idx, pred){
  ...
  // Llave distinta: verificar si el ganador real aparece en la llave del participante
  // y si el participante lo eligió como ganador.
  if(!userM || !userM.home || !userM.away) return {pts: 0, label: 'Llave equivocada: el cruce real fue ' + realM.home + ' vs ' + realM.away};

  const rh = realM.result.home, ra = realM.result.away;
  const realWinner = rh > ra ? realM.home : (ra > rh ? realM.away : (realM.result.penaltyWinner || null));

  const userHasRealWinner = realWinner && (userM.home === realWinner || userM.away === realWinner);
  if(!userHasRealWinner){
    return {pts: 0, label: 'Llave equivocada: el cruce real fue ' + realM.home + ' vs ' + realM.away};
  }

  const rawPred = (pred.knockout[round.key] && pred.knockout[round.key][idx]) || {};
  if(rawPred.home === undefined || rawPred.home === '' || rawPred.away === undefined || rawPred.away === ''){
    return {pts: 0, label: 'Llave equivocada: el cruce real fue ' + realM.home + ' vs ' + realM.away};
  }
  const ph = parseInt(rawPred.home), pa = parseInt(rawPred.away);
  if(isNaN(ph) || isNaN(pa)) return {pts: 0, label: 'Llave equivocada: el cruce real fue ' + realM.home + ' vs ' + realM.away};

  let predWinner;
  if(ph > pa) predWinner = userM.home;
  else if(pa > ph) predWinner = userM.away;
  else predWinner = rawPred.penaltyWinner || null;

  if(predWinner && predWinner === realWinner){
    let clasificadoPts = 0;
    if(round.key === "final"){
      clasificadoPts = STATE.points.campeonFinal;
      const predLoser = predWinner === userM.home ? userM.away : userM.home;
      const realLoser = realWinner === realM.home ? realM.away : realM.home;
      if(predLoser === realLoser) clasificadoPts += STATE.points.subcampeon;
    } else if(round.key === "third"){
      clasificadoPts = STATE.points.tercerPuestoGanador;
    } else {
      clasificadoPts = STATE.points[round.pointsClasificado];
    }
    return {pts: clasificadoPts, label: 'Llave equivocada (rival distinto: ' + realM.home + ' vs ' + realM.away + '), pero acertaste el clasificado: +' + clasificadoPts + ' pts'};
  }
...
function renderPredKnockoutForm(){
  ...
  // ---- Selector de terceros: los 12 clasificados de la persona, elegir 8 para las llaves ----
  // pred.thirdPicks = {"3#1": "grupo", "3#2": "grupo", ...} — igual que antes, mismo formato
  if(!pred.thirdPicks) pred.thirdPicks = {};
  const picks = pred.thirdPicks;

  // Los 12 terceros (uno por grupo), en orden de ranking
  const all12 = thirdsInfo.all; // ya ordenados por pts/DG/GF

  // Mapa grupo → info del tercero
  const thirdsMapAll = {};
  all12.forEach(r => thirdsMapAll[r.group] = r);

  // Slots de llave fijos
  const SLOT_RIVAL = {"3#1":"1E","3#2":"1I","3#3":"1D","3#4":"1G","3#5":"1A","3#6":"1L","3#7":"1B","3#8":"1K"};
  const SLOTS = Object.keys(SLOT_RIVAL);

  // Grupos ya usados en alguna llave
  const usedGroups = new Set(Object.values(picks).filter(Boolean));
  // Grupos asignados a las 8 llaves
  const assignedCount = Object.values(picks).filter(Boolean).length;
  // Grupos sin asignar (de los 12)
  const unassignedGroups = all12.filter(r => !usedGroups.has(r.group));

  html += '<div class="card bracket-thirds-card" style="margin-bottom:14px;">';
  html += '<h3 style="margin:0 0 4px;">Asigná tus terceros a las llaves</h3>';
  html += '<p style="font-size:13px;color:var(--muted);margin:0 0 12px;">Tenés 12 terceros clasificados (uno por grupo). Elegí 8 para las llaves — cada uno solo puede usarse una vez. Los 4 que queden sin asignar no entran.</p>';

  // Indicador de progreso
  html += '<div style="margin-bottom:14px;padding:8px 12px;border-radius:8px;background:'+(assignedCount===8?'#e8f5e9':'#fdf8ee')+';border:1px solid '+(assignedCount===8?'#a5d6a7':'var(--line)')+';">';
  html += '<span style="font-size:13px;font-weight:600;color:'+(assignedCount===8?'#2b7a47':'var(--gold-dark)')+';">'+assignedCount+'/8 llaves asignadas'+(assignedCount===8?' ✓':'')+'</span>';
  if(assignedCount < 8){
    html += '<span style="font-size:12px;color:var(--muted);margin-left:8px;">Faltan '+(8-assignedCount)+'</span>';
  }
  html += '</div>';

  // Tabla de los 12 terceros (referencia visual)
  html += '<details style="margin-bottom:12px;"><summary style="font-size:13px;color:var(--gold-dark);cursor:pointer;font-weight:600;">Ver los 12 terceros proyectados ▾</summary>';
  html += '<div style="margin-top:8px;">';
  all12.forEach((row, idx)=>{
    const isAssigned = usedGroups.has(row.group);
    html += '<div class="third-row" style="opacity:'+(isAssigned?'0.5':'1')+'">';
    ...
    if(isAssigned) html += '<span style="font-size:11px;color:#2b7a47;margin-left:6px;">✓ asignado</span>';
    html += '</div>';
  });
  html += '</div></details>';

  // Selector por cada llave (3#1..3#8)
  SLOTS.forEach(slot=>{
    ...
    all12.forEach(r=>{
      const isUsedElsewhere = usedGroups.has(r.group) && currentPick !== r.group;
      const disabled = isUsedElsewhere ? 'disabled' : '';
      const selected = currentPick === r.group ? 'selected' : '';
      const style = isUsedElsewhere ? 'color:var(--muted);' : '';
      html += '<option value="'+r.group+'" '+selected+' '+disabled+' style="'+style+'">'
        + escapeHtml(r.team)+' (Grupo '+r.group+', '+r.pts+' pts)'
        + (isUsedElsewhere?' — ya asignado':'')
        + '</option>';
    });

    html += '</select>';
    if(currentPick && thirdsMapAll[currentPick]){
      // Comparar equipos reales: el 3° real del grupo del user vs el 3° real del grupo del admin
      const adminPicks = (STATE.knockout && STATE.knockout.adminThirdPicks) || {};
      const adminGroup = adminPicks[slot];
      let badge = '';
      if(adminGroup && isGroupComplete(adminGroup) && isGroupComplete(currentPick)){
        const adminTeam = computeGroupStandings(adminGroup)[2] && computeGroupStandings(adminGroup)[2].team;
      ...
      html += '<span style="font-size:12px;font-weight:600;color:#2b7a47;display:flex;align-items:center;gap:2px;min-width:60px;">✓ '+escapeHtml(thirdsMapAll[currentPick].team)+badge+'</span>';
    } else {
  ...
  // Mostrar los 4 que quedan sin asignar
  if(assignedCount > 0){
    html += '<div style="margin-top:12px;padding-top:10px;border-top:1px solid var(--line);">';
    html += '<span style="font-size:12px;color:var(--muted);font-weight:600;">Sin asignar ('+unassignedGroups.length+'): </span>';
    unassignedGroups.forEach(r=>{
      html += '<span style="display:inline-block;margin:3px 4px;padding:3px 10px;border-radius:999px;background:#f0ede3;font-size:12px;color:var(--muted);">'+escapeHtml(r.team)+' ('+r.group+')</span>';
    });
    html += '</div>';
  }
  ...
  // Mapa slot -> equipo predicho por el participante, para los badges de 1X/2X
  const r32slotsMap = {};
...
function wireBracketInputs(container, pred){
  ...
  treeEl.querySelectorAll("input.bpred").forEach(inp=>{
    inp.addEventListener("change", ()=>{
      ...
      // Si se cambia el marcador y ya no es empate, borrar penaltyWinner
      const entry = pred.knockout[round][idx];
      if(entry.home !== '' && entry.away !== '' && parseInt(entry.home) !== parseInt(entry.away)){
        delete entry.penaltyWinner;
      }
      rebuildTree();
    });
  });

  // Clic en nombre de equipo para elegir ganador por penales (cuando hay empate)
  treeEl.querySelectorAll(".bteam-pen").forEach(span=>{
    span.addEventListener("click", ()=>{
      const round = span.dataset.round, idx = parseInt(span.dataset.idx), team = span.dataset.team;
      if(!pred.knockout[round]) pred.knockout[round] = [];
      if(!pred.knockout[round][idx]) pred.knockout[round][idx] = {};
      const entry = pred.knockout[round][idx];
      // Toggle: si ya era el ganador, desmarcar; si no, marcar
      entry.penaltyWinner = (entry.penaltyWinner === team) ? '' : team;
      rebuildTree();
    });
  });
}
...
function renderAdminBracket(container){
  ...
  // Desplegable con todos los equipos para elegir los participantes de r32.
  function teamSelect(currentVal, side){
    let html = '<select class="text-input admin-team-select" data-side="'+side+'" style="flex:1;font-size:13px;padding:3px 6px;">';
    html += '<option value="">-- Equipo --</option>';
    ALL_TEAMS.forEach(t=>{
  ...
  html += '<p style="font-size:13px;color:var(--muted);margin:4px 0 16px;">Elegí los equipos de cada cruce con los desplegables. Una vez definidos los dieciseisavos, usá "Autocompletar" para propagar ganadores a las siguientes rondas.</p>';

  KNOCKOUT_ROUNDS.forEach((round, ridx)=>{
    html += '<div class="round-label">'+round.label+'</div>';
    STATE.knockout[round.key].forEach((m, midx)=>{
      const isLocked = !!(m.result && (m.result.home !== undefined && m.result.home !== null && m.result.home !== '') && (m.result.away !== undefined && m.result.away !== null && m.result.away !== ''));
      const lockKey = round.key+'-'+midx;

      html += '<div class="bracket-match" data-round="'+round.key+'" data-idx="'+midx+'">';
      html += '<div style="font-size:11px;color:var(--muted);text-transform:uppercase;letter-spacing:0.06em;display:flex;align-items:center;gap:8px;">'+m.id;
      if(isLocked){
        html += '<span style="background:#e8f5e9;color:#2b7a47;border-radius:999px;padding:1px 8px;font-size:10px;font-weight:700;letter-spacing:0.04em;">&#x1F512; CERRADA</span>';
        html += '<button class="admin-unlock-btn" data-lockkey="'+lockKey+'" style="background:none;border:1px solid var(--line);border-radius:4px;padding:1px 8px;font-size:10px;color:var(--muted);cursor:pointer;font-family:Oswald,sans-serif;letter-spacing:0.04em;">Editar</button>';
      }
      html += '</div>';

      if(isLocked){
        // Vista bloqueada: mostrar equipos y resultado fijo
        const isTieNow = m.result.home === m.result.away;
        const penWinNow = m.result.penaltyWinner;
        const winnerHome = isTieNow ? (penWinNow === m.home) : m.result.home > m.result.away;
        const winnerAway = isTieNow ? (penWinNow === m.away) : m.result.away > m.result.home;
        const styleH = winnerHome ? 'font-weight:700;color:var(--pitch);' : 'color:var(--muted);';
        const styleA = winnerAway ? 'font-weight:700;color:var(--pitch);' : 'color:var(--muted);';
        html += '<div style="display:flex;align-items:center;gap:10px;margin-top:6px;padding:8px 10px;background:#f8f7f2;border-radius:6px;border:1px solid var(--line);">';
        html += '<span style="flex:1;text-align:right;font-size:13px;'+styleH+'">'+escapeHtml(m.home||'—')+'</span>';
        html += '<span style="font-family:Oswald,sans-serif;font-size:16px;font-weight:700;color:var(--pitch);min-width:48px;text-align:center;">'+m.result.home+' – '+m.result.away+'</span>';
        html += '<span style="flex:1;font-size:13px;'+styleA+'">'+escapeHtml(m.away||'—')+'</span>';
        if(isTieNow && penWinNow) html += '<span style="font-size:11px;color:var(--gold-dark);margin-left:4px;">&#x1F945; '+escapeHtml(penWinNow)+'</span>';
        html += '</div>';
        // Campos ocultos para que readBracketFromDOM siga funcionando
        html += '<input type="hidden" data-side="home" value="'+escapeHtml(m.home||'')+'">';
        html += '<input type="hidden" data-side="away" value="'+escapeHtml(m.away||'')+'">';
        html += '<input type="hidden" data-side="resHome" value="'+m.result.home+'">';
        html += '<input type="hidden" data-side="resAway" value="'+m.result.away+'">';
        if(isTieNow && penWinNow){
          const penName2 = 'pen-'+round.key+'-'+midx;
          html += '<input type="hidden" class="admin-pen-radio" name="'+penName2+'" data-side="penHome" value="'+escapeHtml(m.home||'')+'" '+(penWinNow===m.home?'checked':'')+' style="display:none">';
          html += '<input type="hidden" class="admin-pen-radio" name="'+penName2+'" data-side="penAway" value="'+escapeHtml(m.away||'')+'" '+(penWinNow===m.away?'checked':'')+' style="display:none">';
        }
      } else {
        ...
        html += '<input type="number" min="0" max="20" data-side="resHome" value="'+(m.result?m.result.home:'')+'" style="width:42px;" class="admin-res-input">';
        html += '<span class="dash">-</span>';
        html += '<input type="number" min="0" max="20" data-side="resAway" value="'+(m.result?m.result.away:'')+'" style="width:42px;" class="admin-res-input">';
        html += '<span style="font-size:11px;color:var(--muted);margin-left:8px;">resultado 90\u2032</span>';
        html += '</div>';
        const isTieNow = m.result && m.result.home === m.result.away;
        const penWinNow = m.result && m.result.penaltyWinner;
        const penBlockDisplay = isTieNow ? '' : 'none';
        html += '<div class="admin-pen-row" style="display:'+penBlockDisplay+';margin-top:6px;align-items:center;gap:8px;flex-wrap:wrap;">';
        html += '<span style="font-size:11px;color:var(--gold-dark);font-weight:600;">&#x1F945; Ganador por penales:</span>';
        const teamH = m.home || 'Local', teamA = m.away || 'Visitante';
        const chkH = (penWinNow === m.home) ? 'checked' : '';
        const chkA = (penWinNow === m.away) ? 'checked' : '';
        const penName = 'pen-'+round.key+'-'+midx;
        html += '<label style="font-size:12px;cursor:pointer;"><input type="radio" class="admin-pen-radio" name="'+penName+'" data-side="penHome" value="'+escapeHtml(m.home)+'" '+chkH+'> '+escapeHtml(teamH)+'</label>';
        html += '<label style="font-size:12px;cursor:pointer;"><input type="radio" class="admin-pen-radio" name="'+penName+'" data-side="penAway" value="'+escapeHtml(m.away)+'" '+chkA+'> '+escapeHtml(teamA)+'</label>';
        html += '</div>';
      }
      html += '</div>';
    });
  ...
  // Mostrar/ocultar el selector de penales en tiempo real
  container.querySelectorAll('.bracket-match').forEach(el=>{
    const resHome = el.querySelector('[data-side="resHome"]');
    const resAway = el.querySelector('[data-side="resAway"]');
    const penRow  = el.querySelector('.admin-pen-row');
    if(!resHome || !resAway || !penRow) return;
    function updatePenRow(){
      const h = resHome.value, a = resAway.value;
      const isTie = h !== '' && a !== '' && parseInt(h) === parseInt(a);
      penRow.style.display = isTie ? '' : 'none';
      if(!isTie) el.querySelectorAll('.admin-pen-radio').forEach(r => r.checked = false);
    }
    resHome.addEventListener('input', updatePenRow);
    resAway.addEventListener('input', updatePenRow);
  });

  // Botón "Editar" — desbloquea la llave borrando el resultado para poder re-editarla
  container.querySelectorAll('.admin-unlock-btn').forEach(btn=>{
    btn.addEventListener("click", ()=>{
      // lockkey formato: "r32-0", "r16-0", "qf-0", "sf-1", "final-0", "third-0"
      const key = btn.dataset.lockkey;
      const lastDash = key.lastIndexOf('-');
      const roundKey = key.substring(0, lastDash);
      const idx = parseInt(key.substring(lastDash + 1));
      if(STATE.knockout[roundKey] && STATE.knockout[roundKey][idx]){
        STATE.knockout[roundKey][idx].result = null;
        renderAdminBracket(container);
        showToast("Llave desbloqueada para edición");
      }
    });
  });
...
function readBracketFromDOM(container){
  container.querySelectorAll('.bracket-match').forEach(el=>{
    ...
    m.home = homeEl ? homeEl.value : m.home;
    m.away = awayEl ? awayEl.value : m.away;
    const rh = el.querySelector('[data-side="resHome"]').value;
    const ra = el.querySelector('[data-side="resAway"]').value;
    if(rh === '' || ra === ''){
      m.result = null;
    } else {
      const rhInt = parseInt(rh), raInt = parseInt(ra);
      // Leer ganador por penales si hay empate
      let penaltyWinner = null;
      if(rhInt === raInt){
        const checkedRadio = el.querySelector('.admin-pen-radio:checked');
        if(checkedRadio) penaltyWinner = checkedRadio.value;
      }
      m.result = {home: rhInt, away: raInt};
      if(penaltyWinner) m.result.penaltyWinner = penaltyWinner;
    }
  });
}
...
function propagateWinners(){
  function getWinner(m){
    ...
    // Empate: usar penaltyWinner si fue definido
    if(m.result.penaltyWinner && (m.result.penaltyWinner===m.home || m.result.penaltyWinner===m.away))
      return m.result.penaltyWinner;
    return "";
  }
  function getLoser(m){
    const w = getWinner(m);
    if(!w || !m.home || !m.away) return "";
  ...
  for(let r=0; r<KNOCKOUT_ROUNDS.length-1; r++){
    ...
    const winners = matches.map(m => getWinner(m));
  ...
  const sfLosers = sf.map(m => getLoser(m));
...
async function renderKnockout(){
  ...
  // Refrescar desde Firestore cada vez que se entra a la página
  const savedKnockout = await storageGet("admin:knockout", true);
  if(savedKnockout) STATE.knockout = savedKnockout;
...
function scoreKnockoutMatch(round, match, pred){
  ...
  const predWinner = ph > pa ? match.home : (pa > ph ? match.away : (pred.penaltyWinner || null));
  const realWinner = rh > ra ? match.home : (ra > rh ? match.away : (match.result.penaltyWinner || null));
...
// REGLA CLAVE: un cruce suma puntos de MARCADOR solo si la llave coincide exactamente
// (mismos dos equipos). Para los puntos de CLASIFICADO, se aplica una regla parcial:
// si el participante predijo que un equipo avanzaría (era el ganador en su bracket en
// esa posición) y ese equipo efectivamente avanzó en el resultado real, recibe los
// puntos de clasificado — aunque el rival en la llave fuera diferente.
// Ejemplo: participante puso "Brasil vs Marruecos" y predijo Brasil ganador;
// la llave real fue "Brasil vs Japón" y Brasil ganó → cobra puntos de clasificado,
// pero NO puntos de marcador (porque la llave era diferente).
function calculateKnockoutScore(pred){
  ...
  KNOCKOUT_ROUNDS.forEach(round=>{
    ...
    realMatches.forEach((realM, idx)=>{
      ...
      const sameMatchup = isSameMatchup(userM, realM);

      if(sameMatchup){
        // Llave correcta: puntuar clasificado + marcador normalmente
        const rawPred = (pred.knockout[round.key] && pred.knockout[round.key][idx]) || {};
        const normPred = normalizeKnockoutPred(userM, realM, rawPred);
        const pts = scoreKnockoutMatch(round, realM, normPred);
        roundPts += pts.total;
      } else {
        // Llave distinta: verificar si el participante predijo correctamente
        // al ganador real (el equipo que clasificó), aunque el rival fuera otro.
        // En ese caso otorgar solo los puntos de clasificado (sin marcador).
        if(!userM || !userM.home || !userM.away) return;
        ...
        // Verificar si el ganador real aparece en la llave del participante
        const userHasRealWinner = userM.home === realWinner || userM.away === realWinner;
        if(!userHasRealWinner) return;

        // Determinar el ganador predicho por el participante en ese cruce
        const rawPred = (pred.knockout[round.key] && pred.knockout[round.key][idx]) || {};
        if(rawPred.home === undefined || rawPred.home === '' || rawPred.away === undefined || rawPred.away === '') return;
        const ph = parseInt(rawPred.home), pa = parseInt(rawPred.away);
        if(isNaN(ph) || isNaN(pa)) return;
        let predWinner;
        if(ph > pa) predWinner = userM.home;
        ...
        // Solo sumar clasificado si el participante eligió al equipo correcto como ganador
        if(predWinner && predWinner === realWinner){
          let clasificadoPts = 0;
          if(round.key === "final"){
            clasificadoPts = STATE.points.campeonFinal;
            // subcampeon: comprobar si el perdedor predicho también coincide con el real
            const predLoser = predWinner === userM.home ? userM.away : userM.home;
            const realLoser = realWinner === realM.home ? realM.away : realM.home;
            if(predLoser === realLoser) clasificadoPts += STATE.points.subcampeon;
          } else if(round.key === "third"){
...
async function renderLeaderboard(){
  ...
  const results = await Promise.all(
    STATE.participants.map(async name => {
  ...
  results.forEach((r, idx)=>{
    ...
    html += '<div class="pts"><strong>'+r.total+'</strong><br><small>'+parts.join(' &middot; ')+'</small></div>';
    html += '</div>';
  });
  html += '</div>';
...
function calculateTotalScore(pred){
  let partidosPts = 0, clasificadosPts = 0, tercerosPts = 0;

  // 1. Puntos por partidos de fase de grupos (marcador exacto o resultado correcto)
  STATE.matches.forEach(m=>{
    const p = pred.matches[m.id];
    partidosPts += scorePrediction(m, p);
  });

  // 2. Puntos por clasificados (1° y 2° de grupo), solo grupos ya completos
  Object.keys(GROUPS).forEach(letter=>{
    ...
    if(predFirst === actualFirst){
      clasificadosPts += STATE.points.campeonGrupo;
    } else if([actualFirst, actualSecond].includes(predFirst)){
      clasificadosPts += STATE.points.dosClasificados;
    }
    if(predSecond === actualSecond){
      clasificadosPts += STATE.points.segundoGrupo;
    } else if([actualFirst, actualSecond].includes(predSecond)){
      clasificadosPts += STATE.points.dosClasificados;
    }
  });

  // 3. Puntos por terceros en llaves: 2 pts por cada slot 3#1..3#8 acertado
  // Se compara el EQUIPO real que quedó 3° en el grupo del admin vs el grupo del participante.
  // No basta con que coincida la letra del grupo — el equipo concreto debe ser el mismo.
  const adminThirdPicks = (STATE.knockout && STATE.knockout.adminThirdPicks) || {};
  const userThirdPicks  = pred.thirdPicks || {};
  ['3#1','3#2','3#3','3#4','3#5','3#6','3#7','3#8'].forEach(slot=>{
    const adminGroup = adminThirdPicks[slot];
    const userGroup  = userThirdPicks[slot];
    if(!adminGroup || !userGroup) return;
    // Ambos grupos deben estar completos para poder comparar equipos reales
    if(!isGroupComplete(adminGroup) || !isGroupComplete(userGroup)) return;
    const adminTeam = computeGroupStandings(adminGroup)[2] && computeGroupStandings(adminGroup)[2].team;
    const userTeam  = computeGroupStandings(userGroup)[2]  && computeGroupStandings(userGroup)[2].team;
    if(adminTeam && userTeam && adminTeam === userTeam){
      tercerosPts += STATE.points.mejorTercero;
    }
  });
...
function buildStandingsTable(standings, pointsMap){
  ...
  standings.forEach((row,idx)=>{
    ...
    let teamCell = escapeHtml(row.team);
    if(pointsMap){
      const info = pointsMap[row.team];
      if(info && info.pts > 0){
        teamCell += ' <span style="background:#2b7a47;color:#fff;border-radius:999px;padding:1px 7px;font-size:11px;font-weight:700;vertical-align:middle;">+'+info.pts+'</span>';
      } else if(info && info.pts === 0 && info.checked){
        teamCell += ' <span style="color:var(--danger);font-weight:700;font-size:13px;vertical-align:middle;">✗</span>';
      }
    }
    html += '<td>'+teamCell+'</td>';
...
function renderPredGroupContent(letter, pred){
  ...
  if(filledCount===0){
    ...
    // Armar pointsMap para las dos primeras filas predichas
    {
      const predFirst  = standings[0] ? standings[0].team : null;
      const predSecond = standings[1] ? standings[1].team : null;
      const realComplete = isGroupComplete(letter);
      const realStandings = realComplete ? computeGroupStandings(letter) : null;
      const actualFirst  = realStandings ? realStandings[0].team : null;
      const actualSecond = realStandings ? realStandings[1].team : null;
      const pointsMap = {};
      if(realComplete && predFirst){
        const pts = predFirst === actualFirst ? STATE.points.campeonGrupo
          : [actualFirst, actualSecond].includes(predFirst) ? STATE.points.dosClasificados : 0;
        pointsMap[predFirst] = {pts, checked: true};
      }
      if(realComplete && predSecond){
        const pts = predSecond === actualSecond ? STATE.points.segundoGrupo
          : [actualFirst, actualSecond].includes(predSecond) ? STATE.points.dosClasificados : 0;
        pointsMap[predSecond] = {pts, checked: true};
      }
      html += buildStandingsTable(standings, realComplete ? pointsMap : null);
    }
