...
function computeUserBracket(pred){
  ...
  // Helper: dado un partido del bracket del admin, obtener el ganador real
  function adminWinner(roundKey, idx){
    const adminM = STATE.knockout && STATE.knockout[roundKey] && STATE.knockout[roundKey][idx];
    if(!adminM || !adminM.result || adminM.home==='' || adminM.away==='') return null;
    const rh = adminM.result.home, ra = adminM.result.away;
    if(rh > ra) return adminM.home;
    if(ra > rh) return adminM.away;
    return adminM.result.penaltyWinner || null;
  }
  function adminLoser(roundKey, idx){
    const adminM = STATE.knockout && STATE.knockout[roundKey] && STATE.knockout[roundKey][idx];
    if(!adminM || !adminM.result || adminM.home==='' || adminM.away==='') return null;
    const w = adminWinner(roundKey, idx);
    if(!w) return null;
    return w === adminM.home ? adminM.away : adminM.home;
  }

  bracket.r32 = r32slots.map((slot, idx)=>{
    const adminM = STATE.knockout && STATE.knockout.r32 && STATE.knockout.r32[idx];
    const hasAdminResult = adminM && adminM.result &&
      adminM.result.home !== undefined && adminM.result.home !== '' &&
      adminM.result.away !== undefined && adminM.result.away !== '';
    if(hasAdminResult){
      // Partido ya jugado: usar equipos y resultado del admin, bloquear
      return {id: slot.id, home: adminM.home, away: adminM.away,
              result: adminM.result, homeSlot: slot.homeSlot, awaySlot: slot.awaySlot,
              lockedByAdmin: true};
    }
  ...
  // (si el admin ya tiene resultado real en una ronda, usarlo para propagar)
  for(let r=0; r<KNOCKOUT_ROUNDS.length; r++){
    ...
    const winners = prevMatches.map((m, i)=>{
      ...
      // Si el partido anterior fue bloqueado por el admin, propagar el ganador real
      if(m.lockedByAdmin){
        const w = m.result.home > m.result.away ? m.home
          : m.result.away > m.result.home ? m.away
          : (m.result.penaltyWinner || "");
        return w;
      }
    ...
    for(let i=0;i<round.numMatches;i++){
      ...
      // Verificar si el admin ya tiene resultado para este partido de rondas posteriores
      const adminM = STATE.knockout && STATE.knockout[round.key] && STATE.knockout[round.key][i];
      const hasAdminResult = adminM && adminM.result &&
        adminM.result.home !== undefined && adminM.result.home !== '' &&
        adminM.result.away !== undefined && adminM.result.away !== '';
      if(hasAdminResult){
        bracket[round.key].push({id: round.key+"-"+(i+1),
          home: adminM.home || home, away: adminM.away || away,
          result: adminM.result, lockedByAdmin: true});
      } else {
  ...
  const sfLosers = sf.map((m,i)=>{
    ...
    return ph > pa ? m.away : m.home;
  });
  const thirdHome = sfLosers[0] || "", thirdAway = sfLosers[1] || "";
  // Verificar si el admin ya tiene resultado para el tercer puesto
  const adminThird = STATE.knockout && STATE.knockout.third && STATE.knockout.third[0];
  const hasAdminThird = adminThird && adminThird.result &&
    adminThird.result.home !== undefined && adminThird.result.home !== '' &&
    adminThird.result.away !== undefined && adminThird.result.away !== '';
  if(hasAdminThird){
    bracket.third = [{id:"third-1", home: adminThird.home || thirdHome, away: adminThird.away || thirdAway,
                      result: adminThird.result, lockedByAdmin: true}];
  } else {
...
function buildBracketTreeHtml(bracket, opts){
  ...
  function cellHtml(team, round, idx, side, m){
    ...
    let scoreHtml = '';
    if(editable && team && !m.lockedByAdmin){
      ...
      const lockBadge = m.lockedByAdmin ? '<span title="Resultado oficial" style="font-size:9px;margin-left:2px;">🔒</span>' : '';
      scoreHtml = '<span class="bscore">'+(side==='home'?m.result.home:m.result.away)+lockBadge+'</span>';
    }
    // Nombre del equipo: si hay empate y es editable (y NO bloqueado), clic para elegir ganador por penales
    let teamLabel;
    if(team && editable && isTie && !m.lockedByAdmin){
...
function renderPredKnockoutForm(){
  ...
  html += '<div id="predBracketTree">'+buildBracketTreeHtml(bracket, {editable:true, thirdPicks: pred.thirdPicks||{}, r32slots: r32slotsMap})+'</div>';
...
function wireBracketInputs(container, pred){
  ...
  function rebuildTree(){
    ...
    treeEl.innerHTML = buildBracketTreeHtml(bracket, {editable:true, thirdPicks: pred.thirdPicks||{}, r32slots: r32slotsMapW});
    wireBracketInputs(container, pred);
  }
