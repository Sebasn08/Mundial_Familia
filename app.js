...
async function renderLeaderboard(){
  ...
  results.forEach((r, idx)=>{
    const b = r.breakdown;
    const gruposPartidos = b.grupos - b.clasificados - b.terceros;
    const parts = ['Partidos: '+gruposPartidos, 'Clasif.: '+b.clasificados, '3eros: '+b.terceros];
    KNOCKOUT_ROUNDS.forEach(rnd=>{ parts.push(ROUND_LABELS2[rnd.key]+': '+(b.rounds[rnd.key]||0)); });
...
function calculateTotalScore(pred){
  let total = 0, gruposPts = 0, clasificadosPts = 0, tercerosPts = 0, eliminatoriasPts = 0;
  ...
  // Mejor tercero: 2 puntos por cada tercero que la persona asigno a una llave
  // y que coincide con el tercero real que el admin definio en esa misma llave.
  // Para ello comparamos pred.thirdPicks (slot -> grupo) contra los equipos
  // reales del bracket del admin en los slots 3#1..3#8 de r32.
  if(STATE.knockout && STATE.knockout.r32 && pred.thirdPicks){
    // Construir mapa slot -> equipo real del admin
    const adminThirdTeams = {};
    R32_FIXTURES.forEach((fx, idx)=>{
      ["home","away"].forEach(side=>{
        if(fx[side].startsWith("3#")){
          const adminMatch = STATE.knockout.r32[idx];
          if(adminMatch) adminThirdTeams[fx[side]] = adminMatch[side] || "";
        }
      });
    });
    // Construir mapa grupo -> equipo real (standings finales)
    const realThirdTeams = {};
    Object.keys(GROUPS).forEach(g=>{
      if(isGroupComplete(g)){
        const st = computeGroupStandings(g);
        if(st[2]) realThirdTeams[g] = st[2].team;
      }
    });
    // Comparar: si la persona puso el grupo G en el slot S,
    // y el equipo real de ese grupo coincide con el equipo del admin en el slot S -> 2 pts
    Object.keys(pred.thirdPicks).forEach(slot=>{
      const pickedGroup = pred.thirdPicks[slot];
      if(!pickedGroup) return;
      const pickedTeam = realThirdTeams[pickedGroup];
      const adminTeam = adminThirdTeams[slot];
      if(pickedTeam && adminTeam && pickedTeam === adminTeam){
        tercerosPts += STATE.points.mejorTercero;
      }
    });
  }

  gruposPts += clasificadosPts + tercerosPts;

  // 3. Knockout rounds — calculado arriba en roundPts, acumular en eliminatoriasPts
  KNOCKOUT_ROUNDS.forEach(r => eliminatoriasPts += (roundPts[r.key] || 0));

  // Puntos por ronda eliminatoria desglosados
  const roundPts = {};
  KNOCKOUT_ROUNDS.forEach(r => roundPts[r.key] = 0);

  if(STATE.knockout){
    const userBracket2 = computeUserBracket(pred);
    KNOCKOUT_ROUNDS.forEach(round=>{
      const realMatches = STATE.knockout[round.key] || [];
      const userMatches = userBracket2[round.key] || [];
      realMatches.forEach(m=>{
        ...
        if(userMatch.home !== m.home) p = {home: userMatch.result.away, away: userMatch.result.home};
        const pts = scoreKnockoutMatch(round, m, {home:String(p.home), away:String(p.away)});
        roundPts[round.key] += pts.total;
      });
    });
  }
  ...
  return {
    ...
    breakdown: {
      ...
      eliminatorias: eliminatoriasPts,
      rounds: roundPts
    }
  };
}
...
function buildStandingsTable(standings, pointsMap){
  ...
  standings.forEach((row,idx)=>{
    ...
    if(pointsMap){
      ...
      let badge = '<td></td>';
      if(info && info.pts > 0){
        badge = '<td><span style="background:#2b7a47;color:#fff;border-radius:999px;padding:2px 8px;font-size:11px;font-weight:700;white-space:nowrap;">+'+info.pts+'</span></td>';
      } else if(info && info.pts === 0 && info.checked){
        badge = '<td><span style="color:var(--danger);font-weight:700;font-size:13px;">✗</span></td>';
      } else {
        badge = '<td></td>';
      }
      html += badge;
...
function renderPredGroupContent(letter, pred){
  ...
  if(filledCount===0){
    ...
    // Armar pointsMap para los dos clasificados predichos
    {
...
function buildQualifierBadge(predicted, actualTeam, label, pts){
  if(!predicted) return '';
  let color='var(--muted)', icon='', ptsBadge='';
  if(actualTeam){
    if(predicted===actualTeam){
      color='#2b7a47'; icon=' ✓';
      if(pts) ptsBadge='<span style="margin-left:5px;background:#2b7a47;color:#fff;border-radius:999px;padding:1px 7px;font-size:11px;font-weight:700;">+'+pts+'</span>';
    } else {
      // Acertó el clasificado pero en puesto incorrecto
      const consolacion = STATE.points.dosClasificados;
      color='#b45309'; icon=' ~';
      if(consolacion) ptsBadge='<span style="margin-left:5px;background:#b45309;color:#fff;border-radius:999px;padding:1px 7px;font-size:11px;font-weight:700;">+'+consolacion+'</span>';
    }
  }
  return '<span style="display:inline-flex;align-items:center;background:#f0ede3;border-radius:999px;padding:3px 10px;margin-right:4px;font-size:12px;font-weight:500;color:'+color+';">'+label+' '+escapeHtml(predicted)+icon+ptsBadge+'</span>';
}
