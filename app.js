...
function renderPredictionsForm(){
  ...
  html += '<p style="font-size:13px;color:var(--muted);margin:4px 0 16px;">Marcador exacto: '+STATE.points.marcadorExacto+' pts &middot; Acertar resultado: '+STATE.points.resultadoCorrecto+' pts &middot; Bono jornada perfecta: +'+STATE.points.jornadaPerfecta+' pts<br>La tabla de posiciones de cada grupo se genera automáticamente según tus predicciones de marcadores.</p>';
  html += '</div>';

  // One card per group: standings table + match inputs
  Object.keys(GROUPS).forEach(letter=>{
    ...
    html += '<div class="card" id="pred-group-card-'+letter+'">';

    // Standings section (will be re-rendered live via JS)
  ...
  // Wire match prediction inputs — update state AND re-render the group's standings live
