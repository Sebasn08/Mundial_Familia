...
function resolveUserR32(pred){
  ...
  // Fallback: N-esimo mejor tercero para slot 3#N
  const autoAssign = all12.map(r => r.group);

  function resolveSlot(slot){
    if(slot.startsWith("3#")){
      const slotN = parseInt(slot.replace("3#","")) - 1;
      const pickedGroup = manualPicks[slot];
      if(pickedGroup && thirdsMapAll[pickedGroup]){
        return thirdsMapAll[pickedGroup].team;
      }
      const autoGroup = autoAssign[slotN];
      return (autoGroup && thirdsMapAll[autoGroup]) ? thirdsMapAll[autoGroup].team : "";
    }
    const pos = slot[0];
    const letter = slot[1];
...
function buildBracketTreeHtml(bracket, opts){
  ...
  function cellHtml(team, round, idx, side, m){
    ...
    return '<div class="bracket-cell '+winnerClass+'"><span class="bteam" title="'+escapeHtml(team||'')+'">'+ (team?escapeHtml(team):label) +badge+'</span>'+scoreHtml+'</div>';
  }
