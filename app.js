...
function checkAuth(){
  ...
  if(saved === "admin")   { AUTH_LEVEL = "admin";   return; }
  if(saved === "general") { AUTH_LEVEL = "general";  return; }
}
...
function applyAuthUI(){
  ...
  if(adminBtn){
    adminBtn.style.display = AUTH_LEVEL === "admin" ? "" : "none";
  }
  // Si venimos de un login recién hecho (no de checkAuth), terminar de inicializar
...
async function init(){
  setupLogin();
  if(!AUTH_LEVEL) return; // el overlay bloquea; initApp se llama desde applyAuthUI al hacer login
  initApp();
}
